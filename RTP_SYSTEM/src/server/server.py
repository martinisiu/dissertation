...
class Server:
    ...
    def handle_rtsp_requests(self):
        ...
        while True:
            ...
            if packet.request_type == RTSPPacket.PLAY:
                ...
                print(self.server_sent_timestamps)
                f=open('../results/server_times.txt','w')
                f.write(str(self.server_sent_timestamps))
                #for ele in self.server_sent_timestamps:
                    #f.write(str(ele) +'\n')
                f.close()
                self._send_rtsp_response(packet.sequence_number)
    ...
    def _handle_video_send(self):
        ...
        while True:
            ...
            # ADDDED SERVER TIME FOR LATENCY MEASUREMENTS

            self.server_sent_timestamps.append(time.time())

...
class Client:
    ...
    def _recv_rtp_packet(self, size=DEFAULT_CHUNK_SIZE) -> RTPPacket:
        ...
        while True:
            try:
                ...
                continue
        #print("Recieved at " + str(time.time()))
    ...
    def _handle_video_receive(self):
        ...
        while True:
            ...
            #ADDED CLIENT TIME FOR LATENCY MEASUREMENTS
            self.client_recv_timestamps.append(time.time())
    ...
    def send_teardown_request(self) -> RTSPPacket:
        ...
        self.is_receiving_rtp = False
        self.is_rtsp_connected = False
        print(self.client_recv_timestamps)
        f=open('../results/client_times.txt','w')
        f.write(str(self.client_recv_timestamps))
        #for ele in self.client_recv_timestamps:
            #f.write(str(ele)+'\n')
        f.close()
        return response
